...
export let updatedProduct = (product) => async (dispatch, getState) => {
  ...
  try {
    let { data } = await axios.put(`/api/products/${product._id}`, product, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
...
export let deleteProduct = (productId) => async (dispatch, getState) => {
  ...
  try {
    let { data } = await axios.delete(`/api/products/${productId}`, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
